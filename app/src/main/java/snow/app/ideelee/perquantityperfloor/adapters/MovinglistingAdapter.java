package snow.app.ideelee.perquantityperfloor.adapters;
...
import snow.app.ideelee.perquantityperfloor.MovingLiftingBookingPerDay;
import snow.app.ideelee.vehical_module.vehicle.dialog.TagItemAdapter;

public class MovinglistingAdapter extends RecyclerView.Adapter<MovinglistingAdapter.ProductViewHolder> {
    ...
    public MovinglistingAdapter(Context mCtx, List<ServiceProviderList> productList) {
        this.mCtx = mCtx;
        this.productList = productList;
    }

    @Override
    public MovinglistingAdapter.ProductViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        //inflating and returning our view holder
        LayoutInflater inflater = LayoutInflater.from(mCtx);
        final View view = inflater.inflate(R.layout.perquantityperfloor, null);
...
//                Intent intent=new Intent(mCtx, BookingAppointment.class);
        ...
        return new MovinglistingAdapter.ProductViewHolder(view);
    }

    @Override
    public void onBindViewHolder(MovinglistingAdapter.ProductViewHolder holder, int position) {
...
if (position==1){
    holder.txt_person.setText("Per Item");
}else {
    holder.txt_person.setText("Per Floor");
}
    }
...
txt_person=itemView.findViewById(R.id.perhour);
    ...
    public void initiatePopupwindowperday(View v,TextView txt_person) {
        ...
        if (txt_person.getText().toString().equals("Per Item")){
            txt.setText("$ 15.00 Per Item");
        }else{
            txt.setText("$ 15.00 Per Floor");
        }
